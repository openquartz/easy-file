...
;!function (window, document) { //gulp build: laydate-header
  ...
  var laydate = {
    ...
    , ready: function (callback) {
      var cssname = 'laydate';
      var ver = ''
      var path = (isLayui ? 'modules/' : '') + 'laydate.css?v=' + laydate.v
          + ver;
  ...
  */
  ...
  Class.prototype.lang = function () {
    var that = this
      ...
      , en: {
        ...
        ,
        formatError: ['The date format error<br>Must be followed：<br>',
  ...
  Class.prototype.init = function () {
    ...
    //日期范围的日历面板是否联动
    that.rangeLinked = !!(options.range && options.rangeLinked && (options.type
        === 'date' || options.type === 'datetime'))

    //切换日历联动方式
    that.autoCalendarModel = function () {
      var state = that.rangeLinked;
      that.rangeLinked = (options.range && (options.type === 'date'
          || options.type === 'datetime'))
          && ((!that.startDate || !that.endDate) || (that.startDate
              && that.endDate && that.startDate.year === that.endDate.year
    ...
    //是否自动切换
    that.autoCalendarModel.auto = that.rangeLinked && options.rangeLinked
        === 'auto';
    ...
    lay.each(['min', 'max'], function (i, item) {
      ...
      if (typeof options[item] === 'number') { //如果为数字
        ...
        ymd = [thisDate.getFullYear(), thisDate.getMonth() + 1,
          thisDate.getDate()];
        hms = [thisDate.getHours(), thisDate.getMinutes(),
          thisDate.getSeconds()];
      } else {
  ...
  Class.prototype.render = function () {
    ...
    lay(divFooter).html(function () {
      ...
      if (options.type === 'datetime') {
        html.push('<span lay-type="datetime" class="' + ELEM_TIME_BTN + '">'
            + lang.timeTips + '</span>');
      }
      if (!(!options.range && options.type === 'datetime')
          || options.fullPanel) {
        html.push('<span class="' + ELEM_PREVIEW + '" title="' + lang.preview
            + '"></span>')
      }
      ...
      lay.each(options.btns, function (i, item) {
        ...
        if (isStatic && item === 'clear') {
          title = options.lang === 'cn' ? '重置'
              : 'Reset';
        }
        btns.push(
            '<span lay-type="' + item + '" class="laydate-btns-' + item + '">'
            + title + '</span>');
      });
    ...
    if (options.shortcuts) {
      ...
      lay(divShortcut).html(function () {
        ...
        var timeBtn = lay(that.footer).find('.' + ELEM_TIME_BTN).removeClass(
            DISABLED);
        ...
        if (options.position !== 'static' && !options.range
          ...
          if (type === 'date') {
            ...
            if (lay(elemMain[0]).find('.' + ELEM_MAIN + ' li.' + THIS
                + ':not(.laydate-disabled)')[0]) {
    ...
    lay.each(options.theme, function (index, theme) {
      if (/^#/.test(theme)) {
        ...
        ].join('').replace(/{{id}}/g, that.elemID).replace(/{{theme}}/g,
            theme));
      }
    });
    //快捷栏样式
    if (options.shortcuts && options.range) {
      styleText.push(
          '#{{id}}.layui-laydate-range{width: 628px;}'.replace(/{{id}}/g,
              that.elemID))
    }
    if (styleText.length) {
    ...
    var shade = options.shade ? ('<div class="' + ELEM_SHADE + '" style="'
        + ('z-index:' + (elem.style.zIndex - 1) + '; background-color: '
            + (options.shade[1] || '#000') + '; opacity: ' + (options.shade[0]
                || options.shade)) + '"></div>') : '';
  ...
  Class.prototype.checkDate = function (fn) {
    ...
    var getEndDate = function () {
      if (options.range) {
        that.endDate = that.endDate || lay.extend({}, options.dateTime,
            function () {
              var obj = {}
    ...
    if (typeof value === 'string' && value) {
      if (that.EXP_IF.test(value)) { //校验日期格式
        ...
        that.hint(lang.formatError[0] + (
            options.range ? (options.format + ' ' + that.rangeStr + ' '
    ...
    //校验右侧面板是否在可选日期区间
    if (options.range) {
      if (that.getDateTime(that.endDate) < that.getDateTime(options.min)
    ...
    that.startDate = that.startDate || value && lay.extend({},
        options.dateTime); // 有默认值才初始化startDate
    that.autoCalendarModel.auto && that.autoCalendarModel();
    that.endState = !options.range || !that.rangeLinked || !!(that.startDate
        && that.endDate); // 初始化选中范围状态
  ...
  Class.prototype.mark = function (td, YMD) {
    ...
    mark && td.find('div').html(
        '<span class="laydate-day-mark">' + mark + '</span>');
  ...
  Class.prototype.limit = function (opts) {
    ...
    var options = that.config;
    var timestrap = {}
    var dateTime = opts.index > (opts.time ? 0 : 41) ? that.endDate
        : options.dateTime;
    var isOut;
  ...
  Class.prototype.calendar = function (value, index, type) {
    ...
    if (dateTime.year < LIMIT_YEAR[0]) {
      dateTime.year = LIMIT_YEAR[0], that.hint(
          lang.invalidDate);
    }
    if (dateTime.year > LIMIT_YEAR[1]) {
      dateTime.year = LIMIT_YEAR[1], that.hint(
          lang.invalidDate);
    }
    ...
    if (options.lang === 'cn') {
      ...
      lay(elemYM[1]).attr('lay-type', 'month').html(
          (dateTime.month + 1) + ' 月');
    } else {
    ...
    //标记选择范围
    if (options.range && !isAlone && type !== 'init') {
      that.stampRange();
    }
  ...
  Class.prototype.list = function (type, index) {
    var that = this
        , options = that.config
        , dateTime = that.rangeLinked ? options.dateTime : [options.dateTime,
      that.endDate][index]
        , lang = that.lang()
        , isAlone = options.range && options.type !== 'date' && options.type
        !== 'datetime' //独立范围选择器
    ...
    //生成时间列表
      ...
      lay.each([24, 60, 60], function (i, item) {
        ...
        lay.each(new Array(item), function (ii) {
          childUL.push(
              '<li' + (that[startEnd][hms[i]] === ii ? ' class="' + THIS + '"'
              : '') + '>' + lay.digit(ii, 2) + '</li>');
        });
    ...
    if (type === 'year' || type === 'month') {
      ...
      lay(ul).find('li').on('click', function () {
        ...
        if (!options.range && options.autoConfirm) {
          if ((options.type === 'month' && type === 'month') || (options.type
              === 'year' && type === 'year')) {
            that.setValue(that.parse()).done().remove();
          }
        }

        (that.autoCalendarModel.auto && !that.rangeLinked) ? that.choose(
            lay(elemCont).find('td.layui-this'), index) : (that.endState
            && that.done(null, 'change'));
      ...
      scroll();
      span.innerHTML = options.range ? [lang.startTime, lang.endTime][index]
          : lang.timeTips;
      ...
      lay(ul).find('ol').each(function (i) {
        ...
        lay(ol).find('li').on('click', function () {
          ...
          setTimeStatus();
          scroll();
          (that.endDate || options.type === 'time' || (options.type
              === 'datetime' && options.fullPanel)) && that.done(null,
              'change');
  ...
  Class.prototype.setBtnStatus = function (tips, start, end) {
    ...
    if (options.range && options.type !== 'time') {
      ...
      isOut = !that.endState || that.newDate(start).getTime() > that.newDate(
          end).getTime();
      ...
      if (tips && isOut) {
        that.hint(
            typeof tips === 'string' ? lang.timeout.replace(/日期/g, tips)
                : lang.timeout
        );
      }
    }
  ...
  Class.prototype.preview = function () {
    ...
    var elemPreview = lay(that.elem).find('.' + ELEM_PREVIEW)
        , value = options.range ? ((that.rangeLinked ? that.endState
        : that.endDate) ? that.parse() : '') : that.parse();
  ...
  Class.prototype.stampRange = function () {
    ...
    if (options.range && !that.endState) {
      lay(that.footer).find(
          ELEM_CONFIRM).addClass(DISABLED);
    }
    ...
    lay.each(tds, function (i, item) {
      ...
      if (thisTime === startTime || thisTime === endTime) {
        (that.rangeLinked || (!that.rangeLinked && (i < 42 ? thisTime
            === startTime : thisTime === endTime))) &&
        lay(item).addClass(
            lay(item).hasClass(ELEM_PREV) || lay(item).hasClass(ELEM_NEXT)
                ? ELEM_SELECTED
                : THIS
  ...
  Class.prototype.done = function (param, type) {
    ...
    typeof options[type || 'done'] === 'function' && options[type
    || 'done'].apply(options, param);
    ...
    if (options.range) {
      ...
      lay.each(['startTime', 'endTime'], function (i, item) {
        ...
        if (index === i) {
          ...
          if (that.getDateTime(lay.extend({}, dateTime, that[item]))
          ...
          } else if (that.getDateTime(lay.extend({}, dateTime, that[item]))
      ...
      if (that.endState && !that.limit({date: that.thisDateTime(1 - index)})) {
        ...
        if ((isChange || that.rangeLinked && that.endState) && that.newDate(
            that.startDate) > that.newDate(that.endDate)) {
          var isSameDate = that.startDate.year === that.endDate.year
              && that.startDate.month === that.endDate.month
              && that.startDate.date === that.endDate.date;
          // 判断是否反选
          var startDate = that.startDate;
          that.startDate = lay.extend({}, that.endDate,
              isSameDate ? {} : that.startTime);
          options.dateTime = lay.extend({}, that.startDate);
          that.endDate = lay.extend({}, startDate,
              isSameDate ? {} : that.endTime);
      ...
      options.autoConfirm ? that.setValue(that.parse()).done().remove()
    ...
    var that = this
      ...
      , confirm: function () {
        if (options.range) {
          if (lay(btn).hasClass(DISABLED)) {
            return that.hint(
                options.type === 'time' ? lang.timeout.replace(/日期/g, '时间')
                    : lang.timeout
            );
          }
        } else {
  ...
  Class.prototype.change = function (index) {
    var that = this
        ...
        , isAlone = options.range && (options.type === 'year' || options.type
        === 'month')
    ...
    return {
      prevYear: function () {
        ...
        if (that.rangeLinked) {
          ...
          that.autoCalendarModel.auto ? that.choose(
              lay(elemCont).find('td.layui-this'), index) : that.done(null,
              'change');
        }
      }
      , prevMonth: function () {
  ...
  thisModule.getThis = function (id) {
    ...
    if (!that && isLayui) {
      layui.hint().error(
          id ? (MOD_NAME + ' instance with ID \'' + id + '\' not found')
