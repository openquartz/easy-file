  ...
  var util = {
    ...
    , countdown: function (endTime, serverTime, callback) {
      var that = this
          ...
          , now = new Date(
          (!serverTime || type) ? new Date().getTime() : serverTime).getTime()
          , count = end - now
          , time = [
    ...
    , toDateString: function (time, format) {
      ...
      var that = this
        ...
        return isNaN(time) ? time : (typeof time === 'string' ? parseInt(time)
            : time)
      }() || new Date())
          , ymd = [
    ...
    , openWin: function (options) {
      var win;
      options = options || {};
      win = options.window || window.open((options.url || ''), options.target,
          options.specs);
      if (options.url) {
        return;
      }
    ...
    , toVisibleArea: function (options) {
      ...
      if (!options.scrollElem[0] || !options.thisElem[0]) {
        return;
      }
    ...
    , event: function (attr, obj, eventType) {
      ...
      _body.off(eventType, '*[' + attr + ']',
          util.event.UTIL_EVENT_CALLBACK[attr])
