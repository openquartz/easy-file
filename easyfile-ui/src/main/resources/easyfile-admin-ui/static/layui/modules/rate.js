...
layui.define('jquery', function (exports) {
  ...
  var $ = layui.jquery
      ...
      //字符常量
      , MOD_NAME = 'rate', ELEM_VIEW = 'layui-rate',
      ICON_RATE = 'layui-icon-rate', ICON_RATE_SOLID = 'layui-icon-rate-solid',
  ...
  Class.prototype.render = function () {
    ...
    if (parseInt(options.value) !== options.value) {
      if (!options.half) {
        options.value = (Math.ceil(options.value) - options.value) < 0.5
            ? Math.ceil(options.value) : Math.floor(options.value)
      }
    }
    ...
    for (var i = 1; i <= options.length; i++) {
      ...
      if (options.half && parseInt(options.value) !== options.value && i
          == Math.ceil(options.value)) {
        temp = temp + '<li><i class="layui-icon layui-icon-rate-half" ' + style
            + '></i></li>';
      } else {
        temp = temp + item;
      }
    }
    temp += '</ul>' + (options.text ? ('<span class="layui-inline">'
        + options.value + '星') : '') + '</span>';
  ...
  Class.prototype.action = function () {
    ...
    _ul.children("li").each(function (index) {
      ...
      othis.on('click', function (e) {
        ...
        if (options.text) {
          _ul.next("span").text(options.value + "星");
        }
      ...
      othis.on('mousemove', function (e) {
        ...
        if (options.half) {
          var x = e.pageX - $(this).offset().left;
          if (x <= wide / 2) {
            othis.children("i").addClass(ICON_RATE_HALF).removeClass(
                ICON_RATE_SOLID)
          }
        }
      })
      ...
      othis.on('mouseleave', function () {
        ...
        if (options.half) {
          if (parseInt(options.value) !== options.value) {
            _ul.children("li:eq(" + Math.floor(options.value) + ")").children(
                "i").addClass(ICON_RATE_HALF).removeClass(ICON_SOLID_RATE)
          }
        }
      })
