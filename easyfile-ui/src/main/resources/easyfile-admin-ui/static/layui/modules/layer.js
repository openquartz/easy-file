...
;!function (window, undefined) {
  ...
  var isLayui = window.layui && layui.define, $, win, ready = {
    getPath: function () {
      ...
      return GLOBAL.layer_dir || jsPath.substring(0,
    ...
    getStyle: function (node, name) {
      var style = node.currentStyle ? node.currentStyle
          : window.getComputedStyle(node, null);
      return style[style.getPropertyValue ? 'getPropertyValue'
          : 'getAttribute'](name);
    },
    ...
    link: function (href, fn, cssname) {
      ...
      (function poll(status) {
        ...
        if (parseInt(ready.getStyle(getLinkElem, 'width')) === 1989) {
          ...
          getLinkElem.getAttribute('lay-status') === STATUS_NAME ? setTimeout(
              poll, delay) : fn();
        } else {
...
//默认内置方法。
  var layer = {
    ...
    ready: function (callback) {
      var cssname = 'layer';
      var ver = '';
      var path = (isLayui ? 'modules/' : 'css/') + 'layer.css?v=' + layer.v
          + ver;
...
//缓存常用字符
  var doms = ['layui-layer', '.layui-layer-title', '.layui-layer-main',
    '.layui-layer-dialog', 'layui-layer-iframe', 'layui-layer-content',
    'layui-layer-btn', 'layui-layer-close'];
  doms.anim = ['layer-anim-00', 'layer-anim-01', 'layer-anim-02',
...
//容器
  Class.pt.vessel = function (conType, callback) {
    ...
    var titleHTML = (config.title ? '<div class="layui-layer-title" style="'
    ...
    callback([
          // 遮罩
          config.shade ? ('<div class="' + doms.SHADE + '" id="' + doms.SHADE
              + times + '" times="' + times + '" style="' + ('z-index:' + (zIndex
                  - 1) + '; ') + '"></div>') : '',

          // 主体
          '<div class="' + doms[0] + (' layui-layer-' + ready.type[config.type])
          + (((config.type == 0 || config.type == 2) && !config.shade)
              ? ' layui-layer-border' : '') + ' ' + (config.skin || '') + '" id="'
          + doms[0] + times + '" type="' + ready.type[config.type] + '" times="'
          + times + '" showtime="' + config.time + '" conType="' + (conType
              ? 'object' : 'string') + '" style="z-index: ' + zIndex + '; width:'
          + config.area[0] + ';height:' + config.area[1] + ';position:'
          + (config.fixed ? 'fixed;' : 'absolute;') + '">'
          + (conType && config.type != 2 ? '' : titleHTML)

          // 内容区
          + '<div' + (config.id ? ' id="' + config.id + '"' : '')
          + ' class="layui-layer-content' + ((config.type == 0 && config.icon
              !== -1) ? ' layui-layer-padding' : '') + (config.type == 3
          ...
          + function () {
            ...
            if (config.type == 3) {
              ...
              // 风格 2
              if (config.icon == 2) {
                return '<div class="layui-layer-loading-2 ' + animClass
                    + '"></div>';
              }
          ...
          + '<div class="layui-layer-setwin">' + function () {
            ...
            if (config.closeBtn) {
              arr.push('<span class="layui-icon layui-icon-close ' + [
                doms[7],
                doms[7] + (config.title ? config.closeBtn : (config.type == 4 ? '1'
                    : '2'))
              ].join(' ') + '"></span>')
            }
          ...
          + (config.btn ? function () {
            ...
            for (var i = 0, len = config.btn.length; i < len; i++) {
              button += '<a class="' + doms[6] + '' + i + '">' + config.btn[i]
                  + '</a>'
            }
  ...
  Class.pt.creat = function () {
    ...
    switch (config.type) {
      ...
      case 4:
        ...
        delete config.title;
        config.tips = typeof config.tips === 'object' ? config.tips
            : [config.tips, true];
    ...
    that.vessel(conType, function (html, titleHTML, moveElem) {
      ...
      config.scrollbar || doms.html.css('overflow', 'hidden').attr('layer-full',
          times);
    }).auto(times);
    ...
    config.type == 2 && layer.ie == 6 && that.layero.find('iframe').attr('src',
        content[0]);
...
//自适应
  Class.pt.auto = function (index) {
    ...
    var area = [layero.innerWidth(), layero.innerHeight()]
      ...
      elem.height(area[1] - titHeight - btnHeight - 2 * (parseFloat(
...
//Tips
  Class.pt.tips = function () {
    ...
    goal.where = [function () { //上
      ...
      tipsG.removeClass('layui-layer-TipsB').addClass('layui-layer-TipsT').css(
          'border-right-color', config.tips[1]);
    }, function () { //右
      goal.tipLeft = goal.left + goal.width + 10;
      goal.tipTop = goal.top;
      tipsG.removeClass('layui-layer-TipsL').addClass('layui-layer-TipsR').css(
          'border-bottom-color', config.tips[1]);
    }, function () { //下
      goal.autoLeft();
      ...
      goal.tipTop = goal.top;
      tipsG.removeClass('layui-layer-TipsR').addClass('layui-layer-TipsL').css(
          'border-bottom-color', config.tips[1]);
    }];
    goal.where[guide - 1]();
    ...
    if (guide === 1) {
      ...
      (goal.top - win.scrollTop() + goal.height + layArea[1] + 8 * 2)
      - win.height() > 0 && goal.where[0]();
    } else if (guide === 4) {
  ...
  Class.pt.callback = function () {
    ...
    layero.find('.' + doms[6]).children('a').on('click', function () {
      ...
      if (index === 0) {
        ...
        var close = config['btn' + (index + 1)] && config['btn' + (index + 1)](
            that.index, layero, that);
  ...
  ready.reselect = function () {
    $.each($('select'), function (index, value) {
      ...
      if (!sthis.parents('.' + doms[0])[0]) {
        (sthis.attr('layer') == 1 && $('.' + doms[0]).length < 1)
...
// 记录宽高坐标，用于还原
  ready.record = function (layero) {
    if (!layero[0]) {
      return window.console && console.error('index error');
    }
    var area = [
  ...
  layer.style = function (index, options, limit) {
    ...
    if (type === ready.type[2]) {
      layero.find('iframe').css({
        height: (typeof options.height === 'number' ? options.height
            : layero.height()) - titHeight - btnHeight
      });
    } else {
      contElem.css({
        height: (typeof options.height === 'number' ? options.height
...
//仿系统prompt
  layer.prompt = function (options, yes) {
    ...
    var prompt,
        content = options.formType == 2 ? '<textarea class="layui-layer-input"'
            + style + placeholder + '></textarea>' : function () {
          return '<input type="' + (options.formType == 1 ? 'password' : 'text')
              + '" class="layui-layer-input"' + placeholder + '>';
        }();
  ...
  layer.tab = function (options) {
    ...
    return layer.open($.extend({
      ...
      content: '<ul class="layui-layer-tabmain">' + function () {
        var len = tab.length, ii = 1, str = '';
        if (len > 0) {
          str = '<li class="layui-layer-tabli ' + THIS + '">' + (tab[0].content
              || 'no content') + '</li>';
          for (; ii < len; ii++) {
            str += '<li class="layui-layer-tabli">' + (tab[ii].content
                || 'no  content') + '</li>';
          }
        }
        return str;
...
//相册层
  layer.photos = function (options, loop, key) {
    ...
    var isObject = !(typeof options.photos === 'string' || options.photos
        instanceof $)
    ...
    loadImage(data[start].src, function (img) {
      ...
      dict.index = layer.open($.extend({
        ...
        area: function () {
          ...
          if (!options.full && (imgarea[0] > winarea[0] || imgarea[1]
              > winarea[1])) {
        ...
        content: '<div class="layui-layer-phimg">'
            + '<img src="' + data[start].src + '" alt="' + alt + '" layer-pid="'
            + data[start].pid + '">'
            + function () {
